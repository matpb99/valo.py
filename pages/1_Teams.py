...
def draw_map_or_match_team_card_by_metric(category, metric):
    ...
    sql_query = """SELECT Team, ROUND(AVG({}),{}) AS {}, Map, LocalTeam || " VS " || VisitTeam AS MatchTeams, Date
                        FROM test_data
                        GROUP BY TeamMapKey
                        ORDER BY AVG({}) DESC
    ...
    if category == "maps":
        team, value, map_name, matchteams =  df_data["Team"][0], df_data[metric][0], df_data["Map"][0], df_data["MatchTeams"][0]
        text = [str(value) +" {}".format(metric), "Most {}{} in One Single Map by Teams in All VCTs".format(prefix, metric), map_name, matchteams]
...
with col9:
    with st.container(border=True):
        draw_map_or_match_team_card_by_metric("maps", "Rating")

with col10:
    with st.container(border=True):
        draw_map_or_match_team_card_by_metric("maps", "ACS")

with col11:
    with st.container(border=True):
        draw_map_or_match_team_card_by_metric("maps", "Kills")

with col12:
    with st.container(border=True):
        draw_map_or_match_team_card_by_metric("maps", "Assists")

col13, col14, col15, col16 = st.columns(4)

with col13:
    with st.container(border=True):
        draw_map_or_match_team_card_by_metric("maps", "Kast")

with col14:
    with st.container(border=True):
        draw_map_or_match_team_card_by_metric("maps", "ADR")

with col15:
    with st.container(border=True):
        draw_map_or_match_team_card_by_metric("maps", "HSRate")

with col16:
    with st.container(border=True):
        draw_map_or_match_team_card_by_metric("maps", "FirstKills")
